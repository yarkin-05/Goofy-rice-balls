  ...
  if (isset($_FILES['image'], $_POST['description'])) {
    ...
    if (!empty($_FILES['image']['tmp_name']) && getimagesize($_FILES['image']['tmp_name'])) {
      if ($_FILES['image']['size'] > 500000) {
        ...
        $stmt = $pdo->prepare('INSERT INTO perfiles_usuario (id_usuario, filepath, informacion_adicional) VALUES (?,?,?)');

        if (!$stmt->execute([$id, $image_path, $_POST['description']])) {
...
?>
<?=template_header("Creating your Profile")?>


<div class="content upload">
  <h3 class="greetings">Welcome <?= $_SESSION['username']?>! </h3> 
  <p class="greetings">Please create a profile</p>
  <h2>User Info</h2>
